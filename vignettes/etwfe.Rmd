...
the `type` argument. This time I'll save the resulting object since I plan to
reuse it in a moment.
...
Here it seems that the disemployment effect of a minimum wage hike is fairly
modest at first, but grows over time. We'll look at ways to present this
information visually in the next section.

Note that `emfx()` also supports other types of aggregations such as 
`"calendar"` (i.e., treatment effect by period) and `"group"` (i.e., treatment
effect by cohort).
...
So far we've limited ourselves to a homogeneous treatment effect, where the
...
vaccine differs across population age groups, or whether a marketing campaign
was equally successful across genders. The ETWFE framework naturally lends
itself to analysis of these kinds of heterogeneous treatment effects.
...
While the simple example here has been limited to average treatment effects
and two groups, note the same logic carries over to richer settings. We can use
the ETWFE workflow to estimate heterogeneous treatment effects by different
aggregations (e.g., event studies) and across groups with many levels.

## Other families

Another key feature of the ETWFE approach---one that sets it apart from other advanced

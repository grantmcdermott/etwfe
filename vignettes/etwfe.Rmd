...
"Treatment" in this dataset means an increase in the minimum wage rate, while 
the main outcome of interest is teen employment. In the examples that follow, 
our goal is to estimate the effect of this minimum wage treatment (`treat`) on 
teen log employment (`lemp`). Notice that the panel ID is at the county
...
the core workflow of the package involves consecutive calls to two complementary 
functions: 1) `etwfe()` and 2) `emfx()`.
...
In other words, an increase in the minimum wage leads to an approximate 5 
percent decrease in teen employment. 

Beyond simple ATEs, `emfx()` also supports other types of aggregations through
the `type` argument. For example, we can use `type = "calendar"` to get ATEs by
period, or `type = "group"` to get ATEs by cohort groups. But the option that
will probably be of use to most people is `type = "event"`, which will recover
dynamic treatment effects _a la_ an event study. This try this out and then save
...
Here it seems that the teen disemployment effect of a minimum wage hike is
fairly modest at first, but grows over time. We'll look at ways to present this
information visually in the next section.
...
successful across genders. The ETWFE framework naturally lends itself to
analysis of these kinds of heterogeneous treatment effects.
...
"xvar" represents a covariate that is *interacted* with treatment, as opposed to 
a regular control variable (which could obviously be included as part of the 
`fml` RHS.] Any subsequent `emfx()` call on this object will automatically
recognize that we want to recover treatment effects by these two distinct 
groups. 
...
It looks like an increase in minimum wage may cause less teen disemployment in
GLS counties than in the rest of the US on average. However, to test this
formally we can invoke the powerful
...
"[i]" denotes the row of the `emfx()` return object. By specifying 
`hypothesis = "b1 = b2"`, we are thus testing whether the treatment estimates 
from row 1 (non-GLS) and row 2 (GLS) are different from one another.
...
Thinking of the **etwfe** workflow as a pair consecutive functional calls, the
first `etwfe()` stage tends to be very fast. We're leveraging the incredible
performance of **fixest** and also taking some shortcuts to avoid wasting time
on nuisance parameters. See the
...
tend to scale linearly with the size of the original data, as well as the number
of interactions from the underlying `etwfe` model object. Without getting too
deep into the weeds, the numerical delta method being used to recover the
ATEs of interest has to estimate two prediction models for *each* coefficient in
the model and then compute their standard errors. So, it's a potentially
expensive operation that can push the computation time for large datasets (> 1m
rows) up to several minutes or longer.
...
even for datasets in excess of a million rows. While the loss of standard errors
might not be an acceptable trade-off for projects where statistical inference is
critical, the good news is this first strategy can still be combined our second
strategy. It turns out that collapsing the data by groups prior to estimating
the marginal effects can yield substantial speed gains of its own. Users can do
this by invoking the `emfx(..., collapse = TRUE)` argument. While the effect
here is not as dramatic as the first strategy, our second strategy does have the
virtue of retaining information about the standard errors. The trade-off this
time, however, is that collapsing our data does lead to a loss in accuracy for
our estimated parameters. On the other hand, testing suggests that this loss in
accuracy tends to be relatively minor, with results equivalent up to the 1st or 
2nd significant decimal place (or even better).

Summarizing, here's a quick plan of attack for you to try if you are worried
...
complete very quickly anyway, but here's a reworking of our earlier event study
...
To put a fine point on it, here's how the collapsed estimates compare with our
...
As I keep reiterating, the Wooldridge ETWFE approach basically involves

...
Other types of aggregations include `"calendar"` (i.e., treatment effect by period) 
and `"group"` (i.e., treatment effect by cohort).
...
are _required_ per the underlying ETWFE theory for nonlinear model families like
Poisson. Still, you can specify unit-level fixed effects for the linear case
through the `ivar` argument. Again, we can easily confirm that this yields the
same estimated treatment effects as the group-level default (although the
